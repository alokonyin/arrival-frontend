...
type ChecklistItem = {
  ...
  deadline_days_before_arrival?: number | null; // Days before arrival date this is due
};
...
export default function StudentChecklistPage() {
  ...
  // Helper: Calculate deadline status and color
  const getDeadlineInfo = (item: ChecklistItem) => {
    if (!item.deadline_days_before_arrival) return null;

    const daysUntilDeadline = daysUntilArrival - item.deadline_days_before_arrival;

    let color = "text-slate-500";
    let bgColor = "bg-slate-100";
    let label = `Due in ${daysUntilDeadline} days`;

    if (daysUntilDeadline < 0) {
      // Overdue
      color = "text-red-700";
      bgColor = "bg-red-100";
      label = `Overdue by ${Math.abs(daysUntilDeadline)} days`;
      ...
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        {activeTab === 'checklist' && (
          <div className="p-6">
            ...
            {/* Countdown Timer */}
            <div className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg px-3 py-2 mt-2">
                          ...
                          >
                            <div className="flex-1">
                              ...
                              {!isDone && (() => {
                                ...
                                if (deadlineInfo) {
                                  return (
                                    <div className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium mt-1 ${deadlineInfo.bgColor} ${deadlineInfo.color}`}>
                                      {deadlineInfo.label}
                                    </div>
                                  );
                                }
